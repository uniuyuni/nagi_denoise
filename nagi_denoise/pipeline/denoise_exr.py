"""Run a NagiV2 checkpoint on an EXR/TIFF image and write diagnostics."""
...
from ..models.nagi_v2 import NagiV2, build_nagi_v2_preset
from .chroma_speckle import PRESETS as CHROMA_SPECKLE_PRESETS
from .chroma_speckle import apply_chroma_speckle_filter
from .flat_luma_chroma_smoother import smooth_flat_luma_chroma
from .luma_hf_shrink import PRESETS as LUMA_HF_PRESETS
from .luma_hf_shrink import apply_luma_hf_shrink
...
def apply_chroma_speckle_preset(
    output: np.ndarray,
    guide: np.ndarray,
    ...
    params = dict(CHROMA_SPECKLE_PRESETS[preset_name])
    return apply_chroma_speckle_filter(
        output,
        guide,
...
def load_model(weights: Path, device: torch.device, state_key: str) -> NagiV2:
...
def run_full_image(model: NagiV2, image: np.ndarray, device: torch.device) -> tuple[np.ndarray, dict[str, np.ndarray]]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Denoise EXR/TIFF with NagiV2.")
    parser.add_argument("--input", required=True)
    parser.add_argument("--weights", required=True)
    parser.add_argument("--output-dir", default="runs/nagi_v2/denoise_exr")
    ...
    parser.add_argument(
        "--post-final-chroma-speckle-preset",
        choices=sorted(POST_CHROMA_SPECKLE_PRESETS),
        default="off",
        help="Final display-chroma speckle guard after luma cleanup.",
    )
    parser.add_argument(
        "--post-luma-hf-preset",
    ...
    if post_chroma_speckle_source is not None:
        output, post_chroma_speckle_stats, post_chroma_speckle_blend = apply_chroma_speckle_preset(
            output, image, post_chroma_speckle_source
        )
    ...
    exr_path = out_dir / f"{name}_nagi_v2.exr"
    tiff_path = out_dir / f"{name}_nagi_v2.tiff"
    preview_path = out_dir / f"{name}_nagi_v2_preview.png"
    ...
    json_path = out_dir / f"{name}_nagi_v2.json"
    ...
    meta = {
        ...
        "outputs": {
            ...
            "post_final_chroma_speckle_blend": str(final_chroma_speckle_blend_path)
            if post_final_chroma_speckle_blend is not None
            else None,
