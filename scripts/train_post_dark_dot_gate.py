"""Train/apply a local gate for the post-v10 dark-dot pass.

The deterministic post-darkdot pass improves the Dance sky but can damage blue
or cyan structures in Occi/Ice. This pilot learns only the blend gate between
the safe v10 output and the stronger post-darkdot candidate.
"""
...
V10_ROOT = RUN_ROOT / "scunet_preset_chooser_v10_tail_chroma_darkdot_lumahf_signed_neutral_blueprotect_auto_outputs"
POST_ROOT = RUN_ROOT / "post_v10_dark_dot_probe"
...
SCENES: dict[str, Scene] = {
    "xt5_occi": Scene(
        ...
        V10_ROOT / "xt5_occi_scunet_preset_chooser_v10_auto.exr",
        POST_ROOT / "xt5_occi_v10_post_darkdot_sky.exr",
        (("face_hair", 1780, 1140), ("bangs", 2170, 850), ("body_shadow", 2900, 3720)),
    ),
    "k5_dance": Scene(
        "k5_dance",
        TEST_PHOTOS / "K-5 Dance noisy.EXR",
        V10_ROOT / "k5_dance_scunet_preset_chooser_v10_auto.exr",
        POST_ROOT / "k5_dance_v10_post_darkdot_sky.exr",
        (("sky_center", 2300, 320), ("dancer_center", 2800, 1200), ("snow_ground", 2100, 2500)),
    ),
    "k5_ice": Scene(
        "k5_ice",
        TEST_PHOTOS / "K-5 Ice noisy.EXR",
        V10_ROOT / "k5_ice_scunet_preset_chooser_v10_auto.exr",
        POST_ROOT / "k5_ice_v10_post_darkdot_sky.exr",
        (("blue_shadow", 2700, 900), ("edge_detail", 1700, 1450), ("ice_center", 2100, 1180)),
    ),
}

POSITIVE_ROIS = {"sky_center"}
SOFT_POSITIVE_ROIS = {"snow_ground"}
...
def impulse_abs(x: np.ndarray, size: int = 3) -> np.ndarray:
    return np.abs(x - median_filter(x, size=int(size), mode="reflect")).astype(np.float32, copy=False)
...
def make_features_and_target(
    ...
    *,
    ...
    cand = display(candidate_linear)
    if ref.shape != base.shape or base.shape != cand.shape:
        raise ValueError(f"shape mismatch ref={ref.shape} base={base.shape} candidate={cand.shape}")

    ref_y = luma(ref, LUMA_SRGB)
    base_y = luma(base, LUMA_SRGB)
    cand_y = luma(cand, LUMA_SRGB)
    sat = saturation(base)
    ...
    shadow_mid = sigmoid01((0.82 - base_y) / 0.20)
    highlight = sigmoid01((base_y - 0.86) / 0.10)
    blue_struct = blue_struct_signal(base)

    low_c = gaussian_filter(chroma(base), sigma=(2.0, 2.0, 0.0), mode="reflect")
    low_chroma_mag = np.sqrt(np.sum(low_c * low_c, axis=2))
    base_luma_imp = impulse_abs(base_y, 3)
    cand_luma_imp = impulse_abs(cand_y, 3)
    luma_benefit = sigmoid01((base_luma_imp - cand_luma_imp + 0.0003) / 0.0018)
    dark_dot = np.maximum(median_filter(base_y, size=3, mode="reflect") - base_y, 0.0)
    dark_gate = sigmoid01((dark_dot - 0.0018) / 0.0018)
    cand_lift = np.maximum(cand_y - base_y, 0.0)
    active = sigmoid01((cand_lift - 0.00020) / 0.00065)
    base_chroma_imp = chroma_impulse(base)
    cand_chroma_imp = chroma_impulse(cand)
    ...
    positive = 0.0
    if roi_name in POSITIVE_ROIS:
        positive = 1.0
    elif roi_name in SOFT_POSITIVE_ROIS:
        positive = float(soft_positive_weight)
    target = (
        positive
        * flat
        * shadow_mid
        * (1.0 - highlight)
        * (1.0 - 0.92 * blue_struct)
        * dark_gate
        * active
        * (0.30 + 0.70 * luma_benefit)
    )
    target = np.clip(target * float(target_gain), 0.0, 1.0)

    candidate_delta = np.mean(np.abs(cand - base), axis=2)
    protect_focus = np.clip(blue_struct * 2.0 + edge * 14.0 + candidate_delta * 80.0, 0.0, 3.5)
    open_focus = np.clip(target * 7.0 + active * dark_gate * positive * 1.8, 0.0, 8.0)
    if positive > 0.0:
        weight = np.clip(0.20 + open_focus + edge * 1.6, 0.20, 7.0)
    elif roi_name is None:
        weight = np.clip(0.08 + protect_focus * 0.35, 0.08, 1.0)
    else:
        weight = np.clip(0.35 + protect_focus, 0.35, 3.8)
    ...
    feats = np.concatenate(
        [
            ...
            dark_gate[..., None],
            cand_lift[..., None],
            base_chroma_imp[..., None],
            cand_chroma_imp[..., None],
            magenta_imp[..., None],
            blue_imp[..., None],
        ],
        axis=2,
    ...
    return feats, target[..., None].astype(np.float32, copy=False), weight[..., None].astype(np.float32), stats
...
class GateDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        *,
        ...
        self.items = []
        for scene in scenes:
            missing = [p for p in (scene.reference, scene.base, scene.candidate) if not p.exists()]
            if missing:
            ...
            self.items.append(
                {
                    ...
                    "candidate": read_image(scene.candidate),
                }
            )
    ...
    def _make_patch(self, item: dict, x: int, y: int, roi_name: str | None) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        cand = crop_with_context(item["candidate"], x, y, self.patch_size, self.context)
        feats, target, weight, _ = make_features_and_target(
            ...
            soft_positive_weight=self.soft_positive_weight,
        )
        c = self.context
        p = self.patch_size
...
class PostDarkDotGate(nn.Module):
...
def save_checkpoint(path: Path, model: PostDarkDotGate, args: argparse.Namespace, step: int) -> None:
    torch.save(
        {
...
def load_model(path: Path, device: torch.device) -> PostDarkDotGate:
    ckpt = torch.load(path, map_location="cpu", weights_only=False)
    model = PostDarkDotGate(width=int(ckpt["width"]), blocks=int(ckpt["blocks"]))
...
def train(args: argparse.Namespace) -> None:
    ...
    model = PostDarkDotGate(width=args.width, blocks=args.blocks).to(device)
    opt = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    meta = {
        "scenes": [scene.name for scene in scenes],
        "dataset_stats": ds.stats,
        "feature_channels": FEATURE_CHANNELS,
        "positive_rois": sorted(POSITIVE_ROIS),
        "soft_positive_rois": sorted(SOFT_POSITIVE_ROIS),
        "args": {k: v for k, v in vars(args).items() if isinstance(v, (str, int, float, bool, type(None)))},
    }
    ...
    with log_path.open("a", encoding="utf-8") as log:
        log.write(f"\n=== train post-darkdot-gate steps={args.steps} device={device} ===\n")
        for step, batch in enumerate(dl, start=1):
            ...
            pred = model(features)
            loss_gate = (F.smooth_l1_loss(pred, target, beta=0.035, reduction="none") * weight).mean()
            ...
            blue_struct = features[:, 17:18]
            edge = features[:, 16:17]
            loss_protect = torch.mean(pred.square() * (blue_struct * 2.4 + edge * 1.2)) * float(args.protect_weight)
            loss = loss_gate + loss_smooth + loss_mean + loss_protect
            ...
            if step == 1 or step % args.log_every == 0:
                ...
                msg = (
                    ...
                    f"mean={float(loss_mean.detach()):.6f} protect={float(loss_protect.detach()):.6f} "
            ...
            if args.save_every > 0 and step % args.save_every == 0:
                save_checkpoint(out_dir / f"post_dark_dot_gate_step_{step:06d}.pt", model, args, step)
        save_checkpoint(out_dir / "post_dark_dot_gate_final.pt", model, args, args.steps)
        log.write(f"wrote {out_dir / 'post_dark_dot_gate_final.pt'}\n")
    print(f"wrote {out_dir / 'post_dark_dot_gate_final.pt'}")
...
def apply(args: argparse.Namespace) -> None:
    ...
    out = base * (1.0 - gate[..., None]) + candidate * gate[..., None]
    name = args.name or f"{scene.name}_post_dark_dot_gate"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train/apply learned post-darkdot blend gate.")
    sub = parser.add_subparsers(dest="cmd", required=True)
    p_train = sub.add_parser("train")
    p_train.add_argument("--output-dir", required=True)
    p_train.add_argument("--scenes", default="xt5_occi,k5_dance,k5_ice")
    p_train.add_argument("--steps", type=int, default=260)
    ...
    p_train.add_argument("--lr", type=float, default=1.6e-4)
    p_train.add_argument("--weight-decay", type=float, default=1.0e-4)
    p_train.add_argument("--smooth-weight", type=float, default=0.020)
    p_train.add_argument("--mean-weight", type=float, default=0.080)
    p_train.add_argument("--protect-weight", type=float, default=0.090)
    p_train.add_argument("--target-gain", type=float, default=1.35)
    p_train.add_argument("--soft-positive-weight", type=float, default=0.20)
    ...
    p_train.add_argument("--seed", type=int, default=424242)
    p_train.add_argument("--log-every", type=int, default=40)
    p_train.add_argument("--save-every", type=int, default=130)
    ...
    p_apply.add_argument("--target-gain", type=float, default=1.35)
    p_apply.add_argument("--soft-positive-weight", type=float, default=0.20)
