"""Distill frequency-split pseudo teachers as opponent-axis chroma correction.

This is stricter than ``train_frequency_chroma_distiller.py``. The model does
not output free RGB residuals. It predicts three signed opponent-axis
coefficients: magenta/green, blue/yellow, and red/cyan. The RGB chroma residual
is reconstructed from those axes, which makes the tiny teacher signal less
ambiguous.
"""
...
AXES = np.array(
    [
        [0.5, -1.0, 0.5],   # magenta / green
        [-0.5, -0.5, 1.0],  # blue / yellow
        [1.0, -0.5, -0.5],  # red / cyan
    ],
    dtype=np.float32,
)
AXES = AXES / np.maximum(np.linalg.norm(AXES, axis=1, keepdims=True), 1.0e-6)
FEATURE_CHANNELS = 22
...
SCENES: dict[str, Scene] = {
    "k5_dance": Scene(
        ...
        (("sky_existing", 4096, 0), ("sky_center", 2300, 320), ("dancer_center", 2800, 1200)),
    ),
    "k5_ice": Scene(
        "k5_ice",
        ...
        (("ice_center", 2100, 1180), ("blue_shadow", 2700, 900), ("edge_detail", 1700, 1450)),
    ),
    "xt5_cat": Scene(
        "xt5_cat",
        ...
        (("fur_detail", 1808, 556), ("dark_noise", 1200, 900), ("whisker", 1900, 620)),
    ),
    "xt5_occi": Scene(
        "xt5_occi",
        ...
        (("face_center", 2120, 1260), ("hair_detail", 2420, 1040), ("root", 512, 5632)),
    ),
}
...
def axis_values(chroma_rgb: np.ndarray) -> np.ndarray:
    return np.tensordot(chroma_rgb, AXES, axes=([2], [1])).astype(np.float32, copy=False)


def axis_hf(display: np.ndarray, sigma: float = 1.2) -> np.ndarray:
    vals = axis_values(chroma(display))
    low = gaussian_filter(vals, sigma=(float(sigma), float(sigma), 0.0), mode="reflect")
    return np.abs(vals - low).astype(np.float32, copy=False)


def coeffs_to_chroma_delta(coeffs: np.ndarray) -> np.ndarray:
    delta = np.tensordot(coeffs, AXES, axes=([2], [0])).astype(np.float32, copy=False)
...
def make_features_and_target(noisy_linear: np.ndarray, base_linear: np.ndarray, teacher_linear: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, dict[str, float]]:
    ...
    dark = sigmoid01((0.62 - base_y) / 0.20)
    base_axis_hf = axis_hf(base)
    noisy_axis_hf = axis_hf(noisy)
    target_chroma = chroma(teacher) - chroma(base)
    target = axis_values(target_chroma)
    target_abs = np.abs(target)
    axis_noise = np.maximum(base_axis_hf, noisy_axis_hf)
    flat_hint = np.clip((1.0 - texture) * low_sat * (0.70 + 0.30 * dark), 0.0, 1.0)
    weight = np.clip(0.08 + target_abs / 0.004 + axis_noise / 0.020 * flat_hint[..., None], 0.08, 3.0)
    ...
    stats = {
        "target_abs_mean": float(np.mean(target_abs)),
        "target_abs_p95": float(np.quantile(target_abs, 0.95)),
        "weight_mean": float(np.mean(weight)),
    }
    return feats, target.astype(np.float32, copy=False), weight.astype(np.float32, copy=False), stats
...
class OpponentDistiller(nn.Module):
    def __init__(self, width: int = 28, blocks: int = 4, max_coeff: float = 0.035) -> None:
        super().__init__()
    ...
    def forward(self, features: torch.Tensor) -> torch.Tensor:
        return torch.tanh(self.tail(self.body(F.gelu(self.head(features))))) * self.max_coeff


class CropDataset(torch.utils.data.Dataset):
    def __init__(self, scenes: list[Scene], patch_size: int, context: int, samples: int, roi_probability: float, seed: int, stats_samples: int) -> None:
        ...
        for scene in scenes:
            ...
            self.items.append({"scene": scene, "noisy": read_image(scene.noisy), "base": read_image(scene.base), "teacher": read_image(scene.teacher), "stats": {}})
    ...
    def _crop(self, arr: np.ndarray, x: int, y: int) -> tuple[np.ndarray, tuple[int, int, int, int]]:
        h, w = arr.shape[:2]
        ...
        return arr[y0:y1, x0:x1], (y - y0, x - x0, y - y0 + self.patch_size, x - x0 + self.patch_size)
    ...
    def _estimate_stats(self, item: dict, stats_samples: int) -> dict[str, float]:
        ...
        return {"target_abs_mean": float(np.mean([v[0] for v in vals])), "weight_mean": float(np.mean([v[1] for v in vals]))}
...
def train(args: argparse.Namespace) -> None:
    ...
    ds = CropDataset(scenes, args.patch_size, args.context, args.steps * args.batch_size, args.roi_probability, args.seed, args.stats_samples)
    dl = torch.utils.data.DataLoader(ds, batch_size=args.batch_size, shuffle=False, num_workers=0, drop_last=True)
    model = OpponentDistiller(width=args.width, blocks=args.blocks, max_coeff=args.max_coeff).to(device)
    ...
    with log_path.open("a", encoding="utf-8") as log:
        log.write(f"\n=== train opponent chroma distiller steps={args.steps} device={device} ===\n")
        for step, batch in enumerate(dl, start=1):
            ...
            pred = model(features)
            loss_main = (F.smooth_l1_loss(pred, target, beta=0.004, reduction="none") * weight).mean()
            ...
            if args.save_every > 0 and step % args.save_every == 0:
                save_checkpoint(out_dir / f"opponent_chroma_distiller_step_{step:06d}.pt", model, args, step)
        save_checkpoint(out_dir / "opponent_chroma_distiller_final.pt", model, args, args.steps)
        log.write(f"wrote {out_dir / 'opponent_chroma_distiller_final.pt'}\n")
    print(f"wrote {out_dir / 'opponent_chroma_distiller_final.pt'}")


def save_checkpoint(path: Path, model: OpponentDistiller, args: argparse.Namespace, step: int) -> None:
    torch.save(
        {
...
def load_model(path: Path, device: torch.device) -> OpponentDistiller:
    ckpt = torch.load(path, map_location="cpu", weights_only=False)
    model = OpponentDistiller(width=int(ckpt["width"]), blocks=int(ckpt["blocks"]), max_coeff=float(ckpt["max_coeff"]))
    model.load_state_dict(ckpt["model"])
    return model.to(device).eval()


@torch.inference_mode()
def predict_tiled(model: OpponentDistiller, device: torch.device, noisy: np.ndarray, base: np.ndarray, *, tile: int, overlap: int) -> tuple[np.ndarray, np.ndarray]:
    height, width = base.shape[:2]
    coeff_acc = np.zeros((height, width, 3), dtype=np.float32)
    ...
    for y0 in range(0, height, stride):
        for x0 in range(0, width, stride):
            ...
            cy1 = cy0 + (y1 - y0)
            cx1 = cx0 + (x1 - x0)
            coeff_acc[y0:y1, x0:x1] += pred[cy0:cy1, cx0:cx1]
            weight_acc[y0:y1, x0:x1] += 1.0
    coeff = coeff_acc / np.maximum(weight_acc, 1.0e-6)
    delta = coeffs_to_chroma_delta(coeff)
    base_display = _display(base)
    out = srgb_to_linear_np(np.clip(base_display + delta, 0.0, 1.0)).astype(np.float32, copy=False)
    return out, coeff.astype(np.float32, copy=False), delta.astype(np.float32, copy=False)
...
def apply(args: argparse.Namespace) -> None:
    ...
    out, coeff, delta = predict_tiled(model, device, noisy, base, tile=args.tile, overlap=args.overlap)
    name = args.name or f"{scene.name}_opponent_chroma_distilled"
    exr_path = out_dir / f"{name}.exr"
    tiff_path = out_dir / f"{name}.tiff"
    preview_path = out_dir / f"{name}_preview.png"
    coeff_path = out_dir / f"{name}_coeff_preview.png"
    ...
    coeff_vis = np.clip(coeff / float(args.coeff_preview_scale) * 0.5 + 0.5, 0.0, 1.0)
    Image.fromarray((coeff_vis * 255.0 + 0.5).astype(np.uint8)).save(coeff_path)
    meta = {
        "scene": scene.name,
        "checkpoint": str(Path(args.checkpoint)),
        "outputs": {"exr": str(exr_path), "tiff": str(tiff_path), "preview": str(preview_path), "coeff": str(coeff_path)},
        "coeff_abs_mean": float(np.mean(np.abs(coeff))),
        "coeff_abs_p95": float(np.quantile(np.abs(coeff), 0.95)),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train/apply opponent-axis chroma distiller.")
    sub = parser.add_subparsers(dest="cmd", required=True)
    p_train = sub.add_parser("train")
    p_train.add_argument("--output-dir", default=str(RUN_ROOT / "opponent_chroma_distiller_pilot"))
    ...
    p_apply.add_argument("--output-dir", default=str(RUN_ROOT / "opponent_chroma_distiller_pilot_outputs"))
    ...
    p_apply.add_argument("--coeff-preview-scale", type=float, default=0.020)
